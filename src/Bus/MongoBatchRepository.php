...
namespace LaraGram\MongoDB\Bus;
...
use LaraGram\MongoDB\Connection;
...
class MongoBatchRepository extends DatabaseBatchRepository implements PrunableBatchRepository
{
    ...
    protected function toBatch($batch): Batch
    {
        return $this->factory->make(
            ...
            $this->toTempora($batch['created_at']),
            $this->toTempora($batch['cancelled_at']),
            $this->toTempora($batch['finished_at']),
        );
    }

    private function getUTCDateTime(): UTCDateTime
    {
        // Using Tempora so the current time can be modified for tests
        return new UTCDateTime(Tempora::now());
    }

    /** @return ($date is null ? null : TemporaImmutable) */
    private function toTempora(?UTCDateTime $date): ?TemporaImmutable
    {
        if ($date === null) {
            return null;
        }

        return TemporaImmutable::createFromTimestampMsUTC((string) $date);
    }
}
