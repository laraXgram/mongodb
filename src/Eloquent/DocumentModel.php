...
use LaraGram\Database\Eloquent\Concerns\HasAttributes;
...
use LaraGram\MongoDB\Query\Builder as QueryBuilder;
use Stringable;
use ValueError;
...
trait DocumentModel
{
    ...
    /**
     * Convert a DateTimeInterface (including Tempora) to a storable UTCDateTime.
     *
    ...
    /**
     * Return a timestamp as Tempora object.
     *
    ...
    {
        // Convert UTCDateTime instances to Tempora.
        if ($value instanceof UTCDateTime) {
    ...
    protected function transformModelValue($key, $value)
    {
        ...
        // to a Tempora or TemporaImmutable instance.
        // @see Model::setAttribute()
        if ($this->hasCast($key) && $value instanceof TemporaInterface) {
    ...
    public function setAttribute($key, $value)
    {
        ...
        if (str_contains($key, '.')) {
            ...
            parent::setAttribute('__LARAGRAM_TEMPORARY_KEY__', $value);

            Arr::set($this->attributes, $key, $this->attributes['__LARAGRAM_TEMPORARY_KEY__'] ?? null);
            unset($this->attributes['__LARAGRAM_TEMPORARY_KEY__']);
    ...
    protected function addCastAttributesToArray(array $attributes, array $mutatedAttributes)
    {
        foreach ($this->getCasts() as $key => $castType) {
            ...
            if ($this->isEnumCastable($key) && (! $castValue instanceof Arrayable)) {
                $castValue = $castValue !== null ? $this->getStorableEnumValueFromLaraGram($this->getCasts()[$key], $castValue) : null;
            }
     ...
     */
    private function getStorableEnumValueFromLaraGram($expectedEnum, $value)
    {
