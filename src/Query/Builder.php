...
namespace LaraGram\MongoDB\Query;
...
use DateTimeZone;
use LaraGram\Database\Query\Builder as BaseBuilder;
...
class Builder extends BaseBuilder
{
    ...
    protected function compileWheres(): array
    {
        ...
        foreach ($wheres as $i => &$where) {
            ...
            // Convert TemporaPeriod to DateTime interval.
            if (isset($where['values']) && $where['values'] instanceof TemporaPeriod) {
                $where['values'] = [
    ...
    {
        $startOfDay = new UTCDateTime(Tempora::parse($where['value'])->startOfDay());
        $endOfDay   = new UTCDateTime(Tempora::parse($where['value'])->endOfDay());
